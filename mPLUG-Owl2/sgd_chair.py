        ...
        for filename in tqdm(os.listdir(input_dir)[:500]):
            if filename.endswith((".jpg", ".jpeg", ".png")):
                ...
                while not is_final:
                    ...
                    accumulated_tokens = tokenizer.encode(accumulated_sentences, return_tensors="pt")
                    accumulated_tokens_length = accumulated_tokens.shape[1]
# NOTE max_token
                    max_new_tokens_length = args.max_new_token - accumulated_tokens_length
                    if max_new_tokens_length <= 0:
                        result = {"id": filename, "question": cur_prompt, "answer": accumulated_sentences, "model": "LLaVA-1.5-7b_vdd_tot"}
                        json.dump(result, f)
                        f.write('\n')
                        f.flush()
                        break
                    ...
                    outputs = [s.rstrip('.') + '.' if s.endswith('.') else s for s in outputs]
                    # import ipdb;ipdb.set_trace()

                    for sub_list in output_ids:
                        sub_list_sliced = sub_list[input_token_len:]
                        if tokenizer.eos_token_id in sub_list_sliced:
                            is_final = True
                            break

                    if(is_final):
                        result = {"id": filename, "question": cur_prompt, "answer": accumulated_sentences, "model": "LLaVA-1.5-7b"}
                        json.dump(result, f)
                        f.write('\n')
                        f.flush()
                        break
                    ...
                    for output in outputs:
                        ...
                        qs_score = "Please analyze the provided image and determine if the following description is accurate. Just say 'yes' if it is, 'no' if it's not, or 'maybe' if you're not sure.\nThe description is: {description} ".format(description=output)
                        # qs_score = "Description: {description}\nPlease analyze the provided image and determine if the description is accurate. Just say 'yes' if it is, 'no' if it's not, or 'maybe' if you're not sure:".format(description=output)
                        ...
                        score_cd = model_outputs['scores'][0]
                        # import ipdb;ipdb.set_trace()
                        
                        all_p_y_cd = []
                        probs_w_token_cd = extract_topk_token_logits(score_cd, tokenizer, apply_softmax=False)
                        all_p_y_cd.append(get_required_token_logits(probs_w_token_cd))
                        scores_cd.append(all_p_y_cd[0][0])

                    max_score_index = scores_cd.index(max(scores_cd))

                    best_output = outputs[max_score_index]
                    accumulated_sentences_before = accumulated_sentences
                    accumulated_sentences = accumulated_sentences + " " + best_output 
                    accumulated_sentences = accumulated_sentences.strip()
                    if accumulated_sentences == accumulated_sentences_before:
                        result = {"id": filename, "question": cur_prompt, "answer": accumulated_sentences, "model": "LLaVA-1.5-7b"}
                        json.dump(result, f)
                        f.write('\n')
                        f.flush()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_dir", type=str, default="/data/c/dataset_for_attack/val2014")#ori_caption.jsonl
    parser.add_argument("--output_file", type=str, default="/data/c/LLaVA-Align/experiments/eval/sampling/chair/mplug_owl2/dse_64_newprompt.jsonl")
...
'''
conda activate mplug_owl2;cd mPLUG-Owl2

python mPLUG-Owl2/sgd_chair.py --max_new_token 512 --output_file /LLaVA-Align/experiments/eval/sampling/chair/mplug_owl2/test.jsonl
